...
export default function Programcilar() {
  ...
  const[aktifProgramcilar,setaktifProgramcılar]=useState(null)
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => { setaktifProgramcılar(dev.id) /* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
